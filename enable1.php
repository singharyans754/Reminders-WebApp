...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        //If the reminder is disabled
        if ($status == "disable") {
            //Update the reminder status to "enable"
            $sql = "UPDATE reminders SET status='enable' WHERE date='$date' AND subject='$subject'";
            if (mysqli_query($conn, $sql)) {
                echo '<script>alert("Reminder is Enabled");</script>';
            } else {
                echo '<script>alert("Error updating reminder: ' . mysqli_error($conn) . '");</script>';
            }
        } else {
            echo '<script>alert("Reminder is Already Enabled");</script>';
        }
    } else {
